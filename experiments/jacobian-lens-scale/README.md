...
| `plot_combined.py`, `plot_per_model.py` | All-model and per-model kurtosis profiles from the stored main-set JSONs (`_long` and `random` files are skipped, they have their own figures). No model is loaded. |
...
| `make_report.py`, `analysis_report.typ`, `analysis_report.pdf` | Per-model reading report (overview, family by family, long-prompt section, summary table) generated from the JSONs and compiled with `typst`. The `.typ` is generated output; edit the script. |
| `analysis_tables.typ`, `analysis_tables.pdf` | The 12 per-model profiles laid out twice, by family and by increasing size, for side-by-side reading. Hand-written Typst; `typst compile analysis_tables.typ`. |
...
typst compile $P/analysis_tables.typ               # analysis_tables.pdf
```
